import {
  ReceiveInternalTokenList,
  ReceiveInternalTokenListResponse,
...
const getReceiveInternalTokenList = async (
  query?: GetReceiveTokenListQuery
): Promise<ReceiveInternalTokenList> => {
  const params: GetConfigTokenListQuery = {
    ...query,
    transactionType: TransactionType.TRANSFER_INTERNAL,
  };
  const response =
    await getConfigTokenList<ReceiveInternalTokenListResponse>(params);
