...
import { ReceiveExternalCurrency, ReceiveInternalCurrency } from "../../../types/expose-type";
...
type ReceiveTokenType = ReceiveExternalCurrency | ReceiveInternalCurrency | undefined;

const ReceiveFunction = forwardRef<ReceiveFunctionRef, ReceiveFunctionProps>((props, ref) => {
  ...
  const [currentStep, setCurrentStep] = useState<ReceiveStep>(ReceiveStep.SELECT_METHOD);
  ...
  const { isAuthenticated, blockchainWallets, masterWallet } = useWalletData();
  ...
  const networks = useMemo(() => {
    ...
    const sameNetwork = newNetWorks.find((item) => item?.id === selectedToken?.id);
  ...
  const receiveAddress = useMemo(() => {
    ...
    return blockchainWallets?.find((item) => item.networkSlug === selectedNetwork?.slug)?.blockchainAddress;
  ...
  const warningMessage = useMemo(() => {
      ...
      return (
        <>
          ...
          <strong style={{ color: theme.palette.text.white }}>
            ...
            <Formatter value={selectedToken?.max_value} unit={selectedToken?.name} />{" "}
          </strong>{" "}
          and <strong style={{ color: theme.palette.text.white }}>select the correct network</strong>, unless you will
          lose your assets.
        </>
      );
    }
    if (selectedMethod === ReceiveMethods.RECEIVE_EXTERNAL && !!amount) {
      return (
        <>
          Please <strong style={{ color: theme.palette.text.white }}>select the correct network</strong>, unless you
  ...
  const validateAmount = (amount: number) => {
    if (amount < (selectedToken?.min_value ?? 0)) {
      setAmountError(`Min ${selectedToken?.min_value} ${selectedToken?.name}`);

      return false;
    }
    if (amount > (selectedToken?.max_value ?? Infinity)) {
      setAmountError(`Max ${selectedToken?.max_value} ${selectedToken?.name}`);
                  ...
                  return (
                    <Fragment key={item}>
                      ...
                      <ListItemCustom
                        ...
                        onClick={() => handleSelectMethod(item as ReceiveMethods)}
                        sx={{
                ...
                >
                  ...
                  >
                    ...
                    >
                      Receive{" "}
                      <strong>
                        {!!amount && <Formatter value={amount} />} {selectedToken?.name}
                      </strong>{" "}
                      {selectedMethod === ReceiveMethods.RECEIVE_INTERNAL ? "Internally" : "Externally"}
                    </Text>
                  </Box>
                  ...
                  >
                    <QRCode value={qrCodeValue} logo={getIcon("ton")} bgColor={"transparent"} />
                    {/* <SafeSvgRenderer
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {selectedMethod === ReceiveMethods.RECEIVE_INTERNAL ? "Internal address" : "Address"}
                      </Text>
                      {selectedMethod === ReceiveMethods.RECEIVE_INTERNAL && (
                        <CustomTooltip trigger={<Icon src={getIcon("tooltip")} width={16} />}>
                          <DialogContentLayout
                            content={
                              <Text
                                sx={{
                              ...
                              >
                                This address is used to receive token by internal transfer. You can not use it for
                                blockchain transaction. If you want to receive token by blockchain, please select the{" "}
                                <strong>Receive External</strong> method instead.
                              </Text>
                            }
                          />
                        </CustomTooltip>
                    ...
                    >
                      <CopyTextComponent value={receiveAddress || ""}>{receiveAddress}</CopyTextComponent>
                    </Text>
                  </Box>
                  ...
                  {!!warningMessage && (
                    ...
                    >
                      <Text sx={{ ...theme.mixins.valueDescription }}>{warningMessage}</Text>
                    </Box>
                  )}
                </Box>
                <DrawerComponent
                ...
                >
                  <ModalLayout>
                    ...
                    >
                      ...
                      />
                      {amountError && <Text sx={{ ...theme.mixins.validationError }}>{amountError}</Text>}
                      <Box
                        sx={{
                          ...theme.mixins.row,
                          width: "100%",
                      ...
                      >
                        {!!amount && <Button.Text onClick={handleUnset}>Unset</Button.Text>}
                        <Button.Primary
                          sx={{ flex: 1 }}
                          onClick={handleContinue}
                          status={!!amountError ? BUTTON_STATUS.DISABLED : BUTTON_STATUS.ENABLED}
                        >
                          Continue
                        </Button.Primary>
                      </Box>
